...
// ─── Page ──────────────────────────────────────────────────────────────────────
export function WhyNomadicClient({ whyNomadicImages }: { whyNomadicImages?: string[] }) {
  const router = useRouter();
  const navigate = (path: string) => router.push(path);

  const chapters = [...CHAPTERS];
  if (whyNomadicImages?.length === 3) {
    if (whyNomadicImages[0]) chapters[0].image = whyNomadicImages[0];
    if (whyNomadicImages[1]) chapters[1].image = whyNomadicImages[1];
    if (whyNomadicImages[2]) chapters[2].image = whyNomadicImages[2];
  }
